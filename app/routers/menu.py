...
@router.patch("/{menu_item_id}/discount", response_model=schemas.MenuItemRead)
def apply_menu_item_discount(menu_item_id: int, discount: float, db: Session = Depends(get_db)):
    db_item = crud.get_menu_item(db, menu_item_id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Menu item not found")
    return crud.apply_menu_item_discount(db, db_item, discount)
